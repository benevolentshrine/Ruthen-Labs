# BORU Features Deep Dive

This document details the core features and internal pipelines of the BORU Security Engine.

---

## 1. The Unix I/O Pipeline (Execution Isolation)

When BORU executes a script, it cannot simply pipe the script's `stdout` directly to the host terminal, as this risks injection attacks and mixes internal security logs with user output.

Instead, BORU uses a strict **I/O Pipeline**:
...
4. **Log Silencing**: The child's environment is injected with `RUST_LOG=off`. This ensures that if the child itself is a Rust binary (or another BORU instance), it won't dump internal tracing logs.
5. **Output Filtering**: Once the child process exits, BORU captures the raw `stdout`. It applies a string filter to remove any internal kernel/seccomp warnings (`boru::cage::sandbox`, `INFO boru::`, etc.) that were emitted by the pre-exec closure before the child executed. 
...
BORU v2.0 implements dual-layered kernel enforcement that cannot be bypassed from user-space, even if the script exploits a vulnerability in the Python interpreter.
...
*   **Targeted Allowlist**: BORU selectively exposes:
    *   `/tmp/momo/workspace` (Read/Write)
    *   `/usr/bin` (Read-Only)
...
*   **Process Protection**: `clone3`, `unshare`, and `ptrace` are blocked to prevent the sandboxed script from creating new namespaces or debugging the parent BORU process.

---

## 3. Pre-Execution Static Gate

Before delegating enforcement to the kernel, BORU performs a static text analysis of the source code. This acts as an immediate fail-safe.
...
If any pattern matches, BORU aborts the execution *before* `fork()` is even called, returning a `BLOCKED` verdict.

---

## 4. Resource Quotas (Cgroups v2)

To prevent an LLM from accidentally (or maliciously) generating code that crashes the host machine (e.g., a memory leak or a fork bomb), BORU utilizes Linux Cgroups v2.

*   **User Slice Delegation**: BORU runs unprivileged. It places child processes into `/sys/fs/cgroup/user.slice/user-1000.service/app.slice`, utilizing systemd's user delegation to manage resources without `sudo`.
...
*   **Immutability**: Log entries are chained together using SHA-256 hashes (similar to a blockchain). If a malicious agent somehow escapes the sandbox and edits a past log entry, the hash chain will break, and the BORU daemon will detect the tamper upon the next startup.

---

## 6. Daemon Mode & TUI Dashboard

BORU is not just a CLI execution tool; it is a long-running service.
